...
class SongViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private val sharedPrefs =
        application.getSharedPreferences("ai_player_prefs", Context.MODE_PRIVATE)
    ...
    private val _themeName =
        MutableStateFlow(
            sharedPrefs.getString("theme_accent_name", "TitaniumGray") ?: "TitaniumGray"
        )
    ...
    private val _sortOrder =
        MutableStateFlow(
            SortOrder.valueOf(
                sharedPrefs.getString("song_sort_order", SortOrder.BY_DATE_ADDED.name)
                    ?: SortOrder.BY_DATE_ADDED.name
            )
        )
    ...
    private val _isSortAscending =
        MutableStateFlow(sharedPrefs.getBoolean("song_sort_ascending", false))
    ...
    val songs: StateFlow<List<Song>> =
        combine(_allSongs, _searchQuery) { songs, query ->
                if (query.isBlank()) {
                    songs
                } else {
                    songs.filter {
        ...
        combine(songs, _sortOrder, _isSortAscending) { filteredSongs, sortOrder, isSortAscending ->
                if (sortOrder == SortOrder.BY_TITLE) {
                    ...
                    val sorted =
                        if (isSortAscending) {
                            mapped.sortedWith(
                                compareBy<SongItemWithLetter> {
                                        if (it.letter == '#') 'Z' + 1 else it.letter
                                    }
                                    .thenBy { it.song.title }
                            )
                        } else {
                            mapped.sortedWith(
                                compareByDescending<SongItemWithLetter> {
                                        if (it.letter == '#') 'A' - 1 else it.letter
                                    }
    ...
    val artistDisplayData: StateFlow<ArtistDisplayData> =
        songs
            .map { filteredSongs ->
                val groupedByArtist = filteredSongs.groupBy { it.artist }
                val mapped =
                    groupedByArtist.map { (artist, songList) ->
                ...
                val scanned =
                    withContext(Dispatchers.IO) {
                        ...
                        if (songsList.isEmpty()) {
                            // Inject 3 demo songs for easy testing in cases where the
                            // emulator/device doesn't have any physical audio files
                            songsList =
                                listOf(
                                    Song(
                                        999991,
    ...
    fun toggleFavorite(song: Song, skipListUpdate: Boolean = false) {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            if (!skipListUpdate) {
                ...
                _allScannedSongs.value =
                    _allScannedSongs.value.map {
                ...
                if (!targetFav) {
                    _currentPlaylistSongs.value =
                        _currentPlaylistSongs.value.filter { it.id != song.id }
                } else {
                    _currentPlaylistSongs.value =
                        _currentPlaylistSongs.value.map {
                            if (it.id == song.id) it.copy(isFavorite = true) else it
                        }
                }
            }
            favoriteSongsCount.value =
                if (targetFav) favoriteSongsCount.value + 1
                else maxOf(0, favoriteSongsCount.value - 1)
    ...
    fun loadPlaylists() {
        viewModelScope.launch { loadPlaylistsInternal() }
    }

    private suspend fun loadPlaylistsInternal() =
        withContext(Dispatchers.IO) {
            ...
            favoriteSongsCount.value =
                favoriteSongs.count { !SongScanner.isPathBlocked(it.path, standardBlocked) }

            // Filter blacklist songs count
            val blacklistSongs = dbHelper.getBlacklistedSongs()
            blacklistSongsCount.value =
                blacklistSongs.count { !SongScanner.isPathBlocked(it.path, standardBlocked) }
                ...
                list.map { playlist ->
                    ...
                    val filteredSongs =
                        songsInPlaylist.filter {
                            !SongScanner.isPathBlocked(it.path, standardBlocked)
                        }
                    playlist.copy(
    ...
    fun deleteSong(context: Context, songId: Long) {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            val nextSong = withContext(Dispatchers.Main) { PlaybackManager.removeFromQueue(songId) }
            if (current?.id == songId && nextSong != null) {
                withContext(Dispatchers.Main) {
                    PlaybackManager.playSong(context, nextSong, forceRestart = true)
                }
    ...
    suspend fun exportPlaylistsJson(): String =
        withContext(Dispatchers.IO) {
    ...
    suspend fun importPlaylistsJson(jsonContent: String): ImportResult =
        withContext(Dispatchers.IO) {
            ...
            try {
                ...
                for (i in 0 until jsonArray.length()) {
                    ...
                    for (j in 0 until songsArray.length()) {
                        ...
                        if (
                            matchedSong == null && !title.isNullOrEmpty() && !artist.isNullOrEmpty()
                        ) {
                        ...
                        if (matchedSong != null) {
                            when {
                                ...
                                isBlacklistList -> {
                                    ...
                                    val success =
                                        dbHelper.addSongToPlaylist(playlistId!!, matchedSong.id)
                                    if (success) {
                                        successCount++
                                    }
                                }
                            ...
                            val generatedId =
                                System.currentTimeMillis() * 1000 + (j * 10 + i) + (1..9).random()
    ...
    fun toggleFavoriteBatch(songs: List<Song>, isFavorite: Boolean) {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            if (!isFavorite) {
                _currentPlaylistSongs.value =
                    _currentPlaylistSongs.value.filterNot { ids.contains(it.id) }
            } else {
                _currentPlaylistSongs.value =
                    _currentPlaylistSongs.value.map {
    ...
    fun deleteSongs(context: Context, songIds: List<Long>) {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            _currentPlaylistSongs.value =
                _currentPlaylistSongs.value.filterNot { songIds.contains(it.id) }
        }
    }
